    ...
    <section class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <div class="form-group"> 
                ...
                <?php if (($user_profile[0]->profile_picture != '' || $user_profile[0]->profile_picture != NULL) && file_exists('uploads/profile_picture/130x130/' . $user_profile[0]->profile_picture)) { ?>
                    <img src="<?php echo base_url() . 'uploads/profile_picture/130x130/' . $user_profile[0]->profile_picture; ?>" width = "128" height = "128">
                <?php } else { ?>
            ...
            <div class="form-group"> 
                ...
                Male <input type="radio" name="gender" value="1" <?php if($user_profile[0]->gender == 1) { echo 'checked';} ?> >&nbsp;&nbsp;
                Female <input type="radio" name="gender" value="0" <?php if($user_profile[0]->gender == 0) { echo 'checked';} ?> >
                <?php echo form_error('gender', '<div class="error">', '</dev>'); ?>
            </div>
            <div class="form-group"> 
                <label>City</label> 
            ...
            <div class="form-group"> 
                <label>Artist Registry</label> <br>
                <?php
                    $artist_cat = [];
                    if(!empty($artist_category)) {
                        foreach ($artist_category as $val) {
                           $artist_cat[] = $val['battle_category']; 
                        }
                    } 
                ?>
                
                <?php foreach($battle_cat as $key => $bc) { ?>
                <h6><input type="checkbox" name="artist_registry[]" <?php if (in_array($key, $artist_cat)) { echo 'checked="checked" '; } ?> value="<?=$key?>">&nbsp;<?=$battle_cat[$key]?></h6>
                    <br> 
                <?php } ?>
            </div>
            <?php } ?>
