...
//	    echo $this->db->last_query();
    ...
    **/  
    ...
    public function sendNotiToVoter($battleId,$winnerId, $looserId='') {
        $sql = "select bv.voter_id, bv.artist_id, br.battle_name from battle_votes bv inner join battle_request br on bv.battle_id = br.battle_request_id where bv.battle_id =$battleId";
        $resultObj = $this->db->query($sql);
	if($resultObj->num_rows() > 0) {
            ...
            if(!empty($resultArr)) {
                foreach ($resultArr as $val) {
                    ...
                    if($val['artist_id']==$winnerId) {
                        ...
                        //the artist you voted for _______= artist name,  has won  the battle ________ =battlename.
                        
                        //send notification to voter
                        $winUsr = $this->user->getSingleUser($val['artist_id']);
                        $notification_msg = 'The artist you voted for '.$winUsr['firstname'].', has won the battle '.$val['battle_name'];
	...
	{
	    $sql = "UPDATE user SET {$field} = {$field} + 1  WHERE id = '".$user_id."'";
	...
	{   
	    ...
	    $this->db->select('(select concat(sl.media,",", sl.title, ",", bm.fk_song_id) from song_library sl inner join battle_media bm on sl.song_id = bm.fk_song_id where bm.battle_id = battle_request.battle_request_id and bm.artist_id = C.id) as challenger_media, '
                    ...
                    . '(select count(l.id) from likes l inner join battle_media bm on l.post_id = bm.fk_song_id where bm.battle_id = battle_request.battle_request_id and bm.artist_id = C.id and l.battle_id = battle_request.battle_request_id) as challenger_like, '
                    . '(select count(l.id) from likes l inner join battle_media bm on l.post_id = bm.fk_song_id where bm.battle_id = battle_request.battle_request_id and bm.artist_id = user.id and l.battle_id = battle_request.battle_request_id) as friend_like, '
    ...
    public function getFerrstyleList() {
        //$this->db->select('user.*');
        $this->db->from('freestyle_beat_library');
