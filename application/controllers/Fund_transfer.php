...
class Fund_transfer extends CI_Controller {
    ...
    function Index() {
        ...
        $this->form_validation->set_rules('amount', 'Amount', 'required|integer|callback_user_wallet_check');
        ...
        { 
            ...
            $PayRequestFields = array(
                ...
                'CancelURL' =>  base_url().'wallet/cancel', // Required.  The URL to which the sender's browser is redirected if the sender cancels the approval for the payment after logging in to paypal.com.  1024 char max.
                ...
                'IPNNotificationURL' => base_url().'wallet/cash_out_user_ipn', // The URL to which you want all IPN messages for this payment to be sent.  1024 char max.
                'Memo' => '', // A note associated with the payment (text, not HTML).  1000 char max
                'Pin' => '', // The sener's personal id number, which was specified when the sender signed up for the preapproval
                'PreapprovalKey' => '', // The key associated with a preapproval for this payment.  The preapproval is required if this is a preapproved payment.  
                'ReturnURL' => base_url().'wallet/success', // Required.  The URL to which the sener's browser is redirected after approvaing a payment on paypal.com.  1024 char max.
                'ReverseAllParallelPaymentsOnError' => '', // Whether to reverse paralel payments if an error occurs with a payment.  Values are:  TRUE, FALSE
                'SenderEmail' => 'booking@gmail.com', // Sender's email address.  127 char max.
            ...
            $Receiver = array(
                ...
                'Email' => $user[0]['paypal_account_id'], // Receiver's email address. 127 char max.
            ...
            if (!$this->paypal_adaptive->APICallSuccessful($PayPalResult['Ack'])) {
                $this->session->set_userdata('fund_error',$PayPalResult['Errors']);
                redirect('wallet');
            } else {    
                ...
                $xml=simplexml_load_string($PayPalResult['XMLRequest']);
                $currencyCode = $xml->currencyCode;
                $senderEmail = $xml->senderEmail;
                
                $xml1 = simplexml_load_string($PayPalResult['XMLResponse']);
                $transactionId = $xml1->paymentInfoList->paymentInfo->transactionId; 
                $amount = $xml1->paymentInfoList->paymentInfo->receiver->amount;
                $receiverEmail = $xml1->paymentInfoList->paymentInfo->receiver->email;
                 
                $dataArray = ['user_id'=>$user_id, 'txn_id'=>$transactionId, 'pay_key'=>$payKey, 'payment_gross'=>$amount, 'currency_code'=>$currencyCode, 'payer_email'=>$senderEmail, 'receiver_email'=>$receiverEmail, 'payment_status'=>$PayPalResult['PaymentExecStatus']];
                //echo '<pre>'; print_r($dataArray); die();
                $this->wallet->setCashOutUserDetails($dataArray, $amount, $user_id);
                $this->session->set_userdata('fund_success','Your account has been credited successfully');
                redirect('wallet');
                }
    ...
    public function user_wallet_check($amount)
	{
            $user_data = $this->user->get_user_by_id();
            
            if(isset($user_data[0]['coins'])) {
                if ($user_data[0]['coins'] < $amount)
		{
			$this->form_validation->set_message('user_wallet_check', 'Your wallet balance not enough!');
			return FALSE;
		}
		else
		{
			return TRUE;
                ...
                $this->form_validation->set_message('user_wallet_check', 'Try again ! Something Wrong.');
                return  FALSE;
            }
    ...
    function Execute_payment() {
        ...
        $ExecutePaymentFields = array(
            'PayKey' => 'AP-432042701H706033B', // The pay key that identifies the payment to be executed.  This is the key returned in the PayResponse message.
            'FundingPlanID' => 'samiran.mmondal-facilitator@gmail.com' 
