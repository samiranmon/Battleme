...
class Forgetpassword extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->form_validation->run() == False) {
            ...
            $result = $this->Usermodel->check_user_data($email);
            if(empty($result)){
                $this->session->set_flashdata('message', 'Your have not registered with us,please sign up to continue');
                ...
                $this->Usermodel->update_user_data($data,$result[0]->id);
                $subject = "Battleme password reset";
                $body = "Hi ".$result[0]->firstname."<br>";
                $body .= "please click on link to reset yor password.<br>";
    ...
    /**
     ...
     public function reset($id,$key){
        $result = $this->Usermodel->get_user_data($id,$key);
        if($result){ 
