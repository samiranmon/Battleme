...
class Admin_dashboard extends CI_Controller {
    ...
    public function update_paypal_setting($id) {
        if ($this->input->post('submit')) {
            $dataParams = $this->input->post();
            unset($dataParams['submit']);
            $status = $this->Adminusermodel->set_paypal_setting($id, $dataParams);
            if ($status) {
                $this->session->set_flashdata('class', 'alert-success');
            ...
            redirect('admin_dashboard/update_paypal_setting/'.$id);
        }

        $data = $this->Adminusermodel->get_paypal_details($id);
        $this->load->view('update_paypal_setting', $data);
    }
